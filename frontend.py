    ...
    if email and password:
        ...
        with col1:
            if st.button("Login", type="primary", use_container_width=True):
                ...
                if response.status_code == 200:
                    ...
                    # Get user info
                    user_response = requests.get("http://127.0.0.1:8000/users/me", headers=get_headers())
        ...
        with col2:
            if st.button("Sign Up", type="secondary", use_container_width=True):
                ...
                response = requests.post("http://127.0.0.1:8000/auth/register", json=signup_data)
...
def upload_page():
    ...
    if uploaded_file and st.button("Share", type="primary"):
        with st.spinner("Uploading..."):
            ...
            response = requests.post("http://127.0.0.1:8000/upload", files=files, data=data, headers=get_headers())
...
def feed_page():
    ...
    response = requests.get("http://127.0.0.1:8000/feed", headers=get_headers())
    if response.status_code == 200:
            ...
            with col2:
                if post.get('is_owner', False):
                    if st.button("🗑️", key=f"delete_{post['id']}", help="Delete post"):
                        # Delete the post
                        response = requests.delete(f"http://127.0.0.1:8000/posts/{post['id']}", headers=get_headers())
                        if response.status_code == 200:
